...
CCC Switch for macOS is code-signed and notarized by Apple. You can download and install it directly without any additional steps. If you encounter issues, try downloading the latest version from the Releases page.
...
2. Add CCC Switch to your antivirus software's whitelist
...
chmod +x CCC-Switch-*.AppImage

# If it still fails, try
./CCC-Switch-*.AppImage --no-sandbox
```
...
**Solutions**:
1. Confirm the file was exported by CCC Switch
...
**Windows**:
- Check taskbar settings to ensure the CCC Switch icon is not hidden
...
Toggle "Lightweight Mode" from the system tray menu. The main window closes, and CCC Switch runs as a tray-only app. Toggle again or click "Open main window" to exit.
...
Yes. Starting from v3.13.0, CCC Switch covers all window re-show paths (normal launch, deep links, singleton activation, tray `show_main`, and Lightweight Mode return). Clicking a `ccccswitch://` link **rebuilds the main window on demand** and displays the import confirmation dialog. The first open is slightly slower than normal state (window rebuild required), but subsequent switches return to normal speed.
