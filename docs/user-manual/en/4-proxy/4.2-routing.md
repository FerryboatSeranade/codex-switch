...
App routing means letting CCC Switch route a specific application's API requests through the local routing service.
...
When routing is enabled, CCC Switch modifies the app's configuration file to point the API endpoint to the local routing service.
...
When disabling routing, CCC Switch will:
...
Before enabling routing, CCC Switch backs up the original configuration and restores it when disabled.
