...
Hermes uses YAML configuration. CCC Switch writes MCP servers to `mcp_servers`, writes editable provider entries to `custom_providers`, reads read-only entries from Hermes' `providers` dict, and updates `model.provider` / `model.default` when switching providers.
...
| Field | Description |
|-------|-------------|
| `models.providers` | Provider configuration (mapped to CCC Switch's "providers") |
...
CCC Switch's priority when modifying configurations:

1. **CCC Switch Database** - Single source of truth (SSOT)
...
- CLI tool configuration files (will be backfilled by CCC Switch)
- CCC Switch's `settings.json`
...
CCC Switch v3.7.0 migrated from JSON files to SQLite:
